...
[f,~] = vl_sift(img_single,'Levels',35,'EdgeThresh',20,'WindowSize',20) ;
%figure
...
for ii = 1:length(feat_pix_cell)
    ...
    for curr_x = lb_x:ub_x
        for curr_y = lb_y:ub_y
            if norm([curr_x; curr_y  ] - center ,2)  <= radius
...
[magnitudes, sortIdxs] = sort(magnitudes,'descend');
feat_pix_cell = feat_pix_cell(sortIdxs);
